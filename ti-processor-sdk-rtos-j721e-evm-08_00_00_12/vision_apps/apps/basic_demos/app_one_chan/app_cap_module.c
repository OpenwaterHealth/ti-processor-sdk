...
static vx_status configure_cap_params(vx_context context, CaptureObj *captureObj, SensorObj *sensorObj, uint32_t instanceID);
static vx_status create_cap_output(vx_context context, CaptureObj *captureObj, SensorObj *sensorObj,  int32_t bufq_depth, uint32_t instanceID);
static vx_status create_cap_error_detection_frame(vx_context context, CaptureObj *captureObj, SensorObj *sensorObj, uint32_t instanceID);
static vx_status configure_cap_output_write(vx_context context, CaptureObj *captureObj, uint32_t instanceID);
...
vx_status app_create_cap_graph_capture(vx_graph graph, CaptureObj *captureObj, uint32_t instanceID)
{
    vx_status status = VX_SUCCESS;
    ...
    if(status == VX_SUCCESS)
    {
        ...
        if(captureObj->en_out_capture_write == 1)
        {
            status = app_create_graph_cap_write_output(graph, captureObj);
        }
    }
    else
    {
...
void app_deinit_cap(CaptureObj *captureObj, vx_int32 bufq_depth)
{
    vx_int32 i;
...
void app_delete_cap(CaptureObj *captureObj)
{
...
vx_status app_create_graph_cap_write_output(vx_graph graph, CaptureObj *captureObj)
{
    vx_status status = VX_SUCCESS;
...
vx_status app_send_cmd_cap_write_node(CaptureObj *captureObj, vx_uint32 enableChanBitFlag, vx_uint32 start_frame, vx_uint32 num_frames, vx_uint32 num_skip,  vx_uint32 scan_type, const char* patient_path, const char* sub_path, uint32_t instanceID)
{
    vx_status status = VX_SUCCESS;
...
vx_status app_send_err_frame(CaptureObj *captureObj)
{
    vx_status status = VX_SUCCESS;
...
tivx_raw_image read_err_image_raw(vx_context context,
...
vx_status app_init_cap(vx_context context, CaptureObj *captureObj, SensorObj *sensorObj, char *objName, int32_t bufq_depth, uint32_t instanceID)
{
    vx_status status = VX_SUCCESS;

    status = configure_cap_params(context, captureObj, sensorObj, instanceID);

    if(status == VX_SUCCESS)
    {
        status = create_cap_output(context, captureObj, sensorObj, bufq_depth, instanceID);
    }

    if(status == VX_SUCCESS)
    {
        status = create_cap_error_detection_frame(context, captureObj, sensorObj, instanceID);
    }

    if(status == VX_SUCCESS)
    {
        status = configure_cap_output_write(context, captureObj, instanceID);
    }
