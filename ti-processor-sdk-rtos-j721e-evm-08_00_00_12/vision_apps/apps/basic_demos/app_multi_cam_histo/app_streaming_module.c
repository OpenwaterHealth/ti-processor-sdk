...
vx_status app_init_streaming(vx_context context, StreamingObj *obj, SensorObj *sensorObj, char *objName, vx_int32 bufq_depth)
{
    ...
    obj->visionStreamInitParams.numChannels = sensorObj->num_cameras_enabled;
    //if (obj->demo_mode == 1)
    //{
    ...
    if (status == VX_SUCCESS)
    {
        ...
        for (int32_t chan = 0; chan < sensorObj->num_cameras_enabled; chan++)
        {
    ...
    if (status == VX_SUCCESS)
    {
        ...
        if (status == VX_SUCCESS)
        {
            ...
            {
                obj->raw_image_arr[q] = vxCreateObjectArray(context, (vx_reference)raw_image, sensorObj->num_cameras_enabled);
